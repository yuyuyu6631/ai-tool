...
def _build_scenario_summary(
    scenario: Scenario, links: list[ScenarioTool]
) -> ScenarioSummary:
...
def list_scenarios(*, db) -> list[ScenarioSummary]:
    ...
    if rows:
        ...
        scenarios = [
            _build_scenario_summary(row, links_by_scenario[row.id]) for row in rows
        ]
    else:
        scenarios = []
...
def _build_ranking_section(
    ranking: Ranking, items: list[RankingItem]
) -> RankingSection:
    result_items = []
...
def list_rankings(*, db) -> list[RankingSection]:
    ...
    if rows:
        ...
        sections = [
            _build_ranking_section(row, items_by_ranking[row.id]) for row in rows
        ]
    else:
        sections = []
