...
def _build_scenario_summary(
    scenario: Scenario, db, prefetched_links: list[ScenarioTool] | None = None
) -> ScenarioSummary:
...
def list_scenarios(*, db) -> list[ScenarioSummary]:
    ...
    scenarios = [
        _build_scenario_summary(row, db, prefetched_links=grouped_links.get(row.id, []))
        for row in rows
    ]
...
def _build_ranking_section(
    ranking: Ranking, db, prefetched_items: list[RankingItem] | None = None
) -> RankingSection:
...
def list_rankings(*, db) -> list[RankingSection]:
    ...
    sections = [
        _build_ranking_section(row, db, prefetched_items=grouped_items.get(row.id, []))
        for row in rows
    ]
