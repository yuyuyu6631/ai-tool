...
def _tool_row_to_summary(tool: Tool) -> ToolSummary:
    ...
    deal_summary = (
        _repair_text(tool.deal_summary).strip()
        or _repair_text(tool.free_allowance_text).strip()
    )
...
def _sort_tools(
    ...
    if view == "latest" or sort == "latest":
        return sorted(
            items,
            key=lambda item: (status_priority(item), item.createdAt, item.id),
            reverse=True,
        )
    if sort == "name":
        return sorted(
            items, key=lambda item: (-status_priority(item), item.name.casefold())
        )
    return sorted(
        items,
        key=lambda item: (
    ...
    for item in categories:
        ...
        category_sections.append(
            HomeCategorySection(
                ...
                items=_sort_tools(
                    tools_by_category.get(canonical_slug, []),
                    sort="featured",
                    view="hot",
                )[:section_size],
...
def _build_scenario_summary(
    scenario: Scenario, db, links: list[ScenarioTool] | None = None
) -> ScenarioSummary:
    if links is None:
...
def list_scenarios(*, db) -> list[ScenarioSummary]:
    ...
    links_by_scenario: dict[int, list[ScenarioTool]] = defaultdict(list)
    if scenario_ids:
        stmt = (
            select(ScenarioTool)
    ...
    scenarios = [
        _build_scenario_summary(row, db, links=links_by_scenario[row.id])
        for row in rows
    ]
...
def _build_ranking_section(
    ranking: Ranking, db, items_rows: list[RankingItem] | None = None
) -> RankingSection:
    if items_rows is None:
        ...
        items_rows = db.scalars(stmt).all()
...
def list_rankings(*, db) -> list[RankingSection]:
    ...
    items_by_ranking: dict[int, list[RankingItem]] = defaultdict(list)
    if ranking_ids:
        stmt = (
            select(RankingItem)
    ...
    sections = [
        _build_ranking_section(row, db, items_rows=items_by_ranking[row.id])
        for row in rows
    ]
