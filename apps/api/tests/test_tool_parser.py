...
@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_valid(mock_getaddrinfo):
    mock_getaddrinfo.return_value = [(2, 1, 6, '', ('8.8.8.8', 0))]
    assert validate_public_url("https://www.google.com") == "https://www.google.com"

@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_invalid_private_ip(mock_getaddrinfo):
    mock_getaddrinfo.return_value = [(2, 1, 6, '', ('127.0.0.1', 0))]
    with pytest.raises(ValueError, match="不允许抓取本机或局域网地址"):
        validate_public_url("http://local.test.com")

def test_validate_public_url_invalid_hostname():
    with pytest.raises(ValueError, match="无效的URL主机名"):
        validate_public_url("http://")

def test_validate_public_url_invalid_scheme():
    with pytest.raises(ValueError, match=r"只支持公开的 http\(s\) 地址"):
