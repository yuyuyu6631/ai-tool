...
@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_integer_ip(mock_getaddrinfo):
...
@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_hex_ip(mock_getaddrinfo):
    mock_getaddrinfo.return_value = [(2, 1, 6, "", ("127.0.0.1", 0))]
    with pytest.raises(ValueError):
...
@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_private_ip(mock_getaddrinfo):
...
@patch("app.services.tool_parser_service.socket.getaddrinfo")
def test_validate_public_url_unresolvable(mock_getaddrinfo):
    import socket

    mock_getaddrinfo.side_effect = socket.gaierror("Name or service not known")
    with pytest.raises(ValueError):
        validate_public_url("http://unresolvable.invalid")
